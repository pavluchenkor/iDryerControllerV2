...
PN532::PN532(PN532Interface &interface)
{
    ...
    _passiveTargetIDDetectionPending = false;
}
...
bool PN532::SAMConfig(void)
{
    ...
    DEBUG_W("SAMConfig irq=1 timeout=0x14");
...
bool PN532::startPassiveTargetIDDetection(uint8_t cardbaudrate)
{
    ...
    _passiveTargetIDDetectionPending = true;
    return 1;
}

/**************************************************************************/
/*!
    Reads a pending ISO14443A detection result after the host has already
    started passive target detection and observed PN532 ready/IRQ.

    @param  uid           Pointer to the array that will be populated
                          with the card's UID (up to 7 bytes)
    @param  uidLength     Pointer to the variable that will hold the
                          length of the card's UID.
    @param  timeout       Max time to wait for the pending response.
    @param  inlist        If set to true, the card will be inlisted

    @returns 1 if everything executed properly, 0 for an error
*/
/**************************************************************************/
bool PN532::readDetectedPassiveTargetID(uint8_t *uid, uint8_t *uidLength, uint16_t timeout, bool inlist)
{
    int16_t status = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout);
    if (status < 0)
    {
        DEBUG_W("readDetectedPassiveTargetID status=%d", status);
    ...
    {
        DEBUG_D("no passive target nbTg=%u", pn532_packetbuffer[0]);
        return 0;
    }
    ...
    DEBUG_D("target detected nbTg=%u atqa=0x%04X sak=0x%02X uidLen=%u",
            pn532_packetbuffer[0],
            sens_res, pn532_packetbuffer[4], pn532_packetbuffer[5]);
...
{
    if (!startPassiveTargetIDDetection(cardbaudrate))
    {
        return 0x0;
    }

    bool found = readDetectedPassiveTargetID(uid, uidLength, timeout, inlist);
    if (!found)
    {
        _passiveTargetIDDetectionPending = false;
    }
...
bool PN532::passiveTargetIDDetectionPending(void) const
{
    return _passiveTargetIDDetectionPending;
}
...
uint8_t PN532::mifareclassic_AuthenticateBlock(uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    ...
    {
        DEBUG_W("MIFARE auth failed status=0x%02X", pn532_packetbuffer[0]);
        return 0;
    }
...
{
    DEBUG_D("MIFARE read block=%u", blockNumber);
    ...
    if (pn532_packetbuffer[0] != 0x00)
    {
        DEBUG_W("MIFARE read failed block=%u status=0x%02X", blockNumber, pn532_packetbuffer[0]);
        return 0;
    }
...
uint8_t PN532::mifareclassic_WriteDataBlock(uint8_t blockNumber, uint8_t *data)
{
    ...
    if (pn532_packetbuffer[0] != 0x00)
    {
        DEBUG_W("MIFARE write failed block=%u status=0x%02X", blockNumber, pn532_packetbuffer[0]);
        return 0;
    }
...
uint8_t PN532::mifareultralight_WritePage(uint8_t page, uint8_t *buffer)
{
    ...
    if (status < 0)
    {
        DEBUG_W("UL write failed page=%u transport=%d", page, status);
        return 0;
    }

    if (pn532_packetbuffer[0] != 0x00)
    {
        DEBUG_W("UL write failed page=%u status=0x%02X", page, pn532_packetbuffer[0]);
        return 0;
    }
