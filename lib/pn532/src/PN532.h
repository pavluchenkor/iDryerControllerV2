...
{
public:
    /** @brief Создаёт PN532 поверх выбранного transport-слоя. */
    PN532(PN532Interface &interface);

    /** @brief Инициализирует transport и будит PN532. */
    void begin(void);
    ...
    */
    ...
    /** @brief Запускает поиск ISO14443A-метки и сразу возвращает управление. */
    bool startPassiveTargetIDDetection(uint8_t cardbaudrate);
    /** @brief Дочитывает результат ранее запущенного поиска ISO14443A-метки. */
    bool readDetectedPassiveTargetID(uint8_t *uid, uint8_t *uidLength, uint16_t timeout = 1000, bool inlist = false);
    /** @brief Совместимый blocking API: start detect + wait + read result. */
    bool readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout = 1000, bool inlist = false);
    /** @brief Возвращает transport-ready состояние PN532. */
    bool isReady(void);
    /** @brief Показывает, что старт detect уже выполнен и ответ ещё не считан. */
    bool passiveTargetIDDetectionPending(void) const;
...
private:
    ...
    bool _passiveTargetIDDetectionPending;
