...
float thermistor_calc_temp(Thermistor *therm, float adc_value) {
    // защита от крайних значений АЦП
    if (adc_value <= 0.0f) adc_value = 1e-6f;
    if (adc_value >= 1.0f) adc_value = 1.0f - 1e-6f;

    // эквивалентное сопротивление плеча с термистором
    float r_div = therm->pullup * adc_value / (1.0f - adc_value);

    // учесть последовательный резистор (если есть)
    float r_ntc = r_div - therm->inline_resistor;
    if (r_ntc <= 0.0f) return NAN;  // иначе logf взорвётся

    float ln_r = logf(r_ntc);

    float inv_t = therm->c1
                + therm->c2 * ln_r
                + therm->c3 * ln_r * ln_r * ln_r;   // Steinhart–Hart

    return (1.0f / inv_t) + KELVIN_TO_CELSIUS;      // K -> °C
}
