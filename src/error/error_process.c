...
static ErrorCallback g_cb = 0;
static uint8_t g_ctrl_mask = 0xFF;          // разрешённые ctrl_id (0..7) по битам
static ErrSeverity g_min_sev = ERRSEV_INFO; // минимальная важность

void error_set_handler(ErrorCallback cb){ g_cb = cb; }
void error_set_min_severity(ErrSeverity s){ g_min_sev = s; }
